...
from auth_common import get_application, test_authentication_enabled, test_authentication_setup_bypassed
...
async def create_application(graph_client: GraphServiceClient, request_app: Application) -> Tuple[str, str]:
    ...
    request_principal = ServicePrincipal(
        app_id=client_id, display_name=app.display_name)
...
async def add_client_secret(graph_client: GraphServiceClient, app_id: str) -> str:
    request_password = AddPasswordPostRequestBody(
        password_credential=PasswordCredential(display_name="WebAppSecret", start_date_time=datetime.datetime.now(
...
def client_app(server_app_id: str, server_app: Application, identifier: int) -> Application:
    return Application(
        ...
        spa=SpaApplication(redirect_uris=[
                           "http://localhost:50505/redirect", "http://localhost:5173/redirect"]),
        required_resource_access=[
            RequiredResourceAccess(
...
async def main():
    ...
    if test_authentication_setup_bypassed():
        print("AZURE_BYPASS_AUTHENTICATION_SETUP is set to true. Skipping authentication setup.")
        exit(0)
    ...
    _, client_app_id, _ = await create_or_update_application_with_secret(
        ...
        request_app=client_app(
            server_app_id, server_app_permission, app_identifier),
    )
