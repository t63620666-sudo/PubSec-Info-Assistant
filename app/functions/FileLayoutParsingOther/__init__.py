...
def PartitionFile(file_extension: str, file_url: str):
    ...
    try:
        ...
        raise UnstructuredError(
            f"An error occurred trying to parse the file: {str(e)}") from e
...
def main(msg: func.QueueMessage) -> None:
    try:
        ...
        statusLog.upsert_document(
            blob_name, f'{function_name} - partitioning complete', StatusClassification.DEBUG)
        ...
        MAX_CHARACTERS = 2750
        chunks = chunk_by_title(elements, multipage_sections=True, new_after_n_chars=NEW_AFTER_N_CHARS,
                                combine_text_under_n_chars=COMBINE_UNDER_N_CHARS, max_characters=MAX_CHARACTERS)
        statusLog.upsert_document(
            blob_name, f'{function_name} - chunking complete. {len(chunks)} chunks created', StatusClassification.DEBUG)
        ...
        statusLog.upsert_document(
            blob_name, f'{function_name} - chunking stored.', StatusClassification.DEBUG)
