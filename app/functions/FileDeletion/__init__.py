...
def delete_content_blobs(blob_service_client: BlobServiceClient, deleted_blob: str) -> dict:
    ...
    content_list = content_container_client.list_blobs(
        name_starts_with=deleted_blob)
    for blob in content_list:
        chunked_blobs_to_delete[blob.name] = None
    logging.debug("Total number of chunked blobs to delete - %s",
                  str(len(chunked_blobs_to_delete)))
...
def delete_search_entries(deleted_content_blobs: dict) -> None:
    ...
    for file_path in deleted_content_blobs.keys():
        search_id_list_to_delete.append(
            {"id": status_log.encode_document_id(file_path)})

    logging.debug("Total Search IDs to delete: %s",
                  str(len(search_id_list_to_delete)))
    ...
    for blob in deleted_blobs:
        try:
            ...
            if status_log.read_file_state(f"upload/{format(blob)}") == State.DELETED:
                ...
                deleted_content_blobs = delete_content_blobs(
                    blob_service_client, blob)
                logging.info("%s content blobs deleted.",
                             str(len(deleted_content_blobs)))
            ...
            logging.info("An exception occured with doc %s: %s",
                         blob_name, str(err))
            doc_base = os.path.basename(blob)
            doc_path = f"upload/{format(blob)}"
            temp_doc_id = status_log.encode_document_id(doc_path)
