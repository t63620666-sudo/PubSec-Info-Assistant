...
def main(myblob: func.InputStream):
    ...
    try:
        ...
        statusLog.upsert_document(myblob.name, 'Pipeline triggered by Blob Upload',
                                  StatusClassification.INFO, State.PROCESSING, False)
        statusLog.upsert_document(
            myblob.name, f'{function_name} - FileUploadedFunc function started', StatusClassification.DEBUG)
        ...
        if file_extension == 'pdf':
            ...
            statusLog.upsert_document(
                myblob.name, error_message, StatusClassification.ERROR, State.SKIPPED)
        ...
        upload_blob_client = blob_client.get_blob_client(
            container=azure_blob_upload_container, blob=myblob_filename)
        ...
        if 'do_not_process' in metadata:
            if do_not_process == 'true':
                statusLog.upsert_document(
                    myblob.name, 'Further procesiang cancelled due to do-not-process metadata = true', StatusClassification.DEBUG, State.COMPLETE)
                return
        ...
        for blob in blobs:
            blob_client.get_blob_client(
                container=azure_blob_content_container, blob=blob.name).delete_blob()
            search_id_list_to_delete.append(
                {"id": statusLog.encode_document_id(blob.name)})
        ...
        blob_service_client = BlobServiceClient(
            azure_blob_endpoint, credential=azure_credential)
        upload_container_client = blob_service_client.get_container_client(
            azure_blob_upload_container)
        ...
        statusLog.upsert_document(
            myblob.name, f'{function_name} - {file_extension} file sent to submit queue. Visible in {backoff} seconds', StatusClassification.DEBUG, State.QUEUED)

    except Exception as err:
        statusLog.upsert_document(
            myblob.name, f"{function_name} - An error occurred - {str(err)}", StatusClassification.ERROR, State.ERROR)
