...
if [ -z "$resource_group" ] || [ -z "$acr_name" ] || [ -z "$enrichapp_name" ] || [ -z "$image_name" ]; then
  echo "Usage: $0 <resource_group> <acr_name> <enrichapp_name> <image_name>"
  exit 1
fi
...
echo "Building Docker image: $enrichapp_name"
echo -e "\n"
docker build -t "$enrichapp_name" "${DIR}" --build-arg BUILDKIT_INLINE_CACHE=1
...
docker tag "$enrichapp_name" "$image"
docker tag "$enrichapp_name" "$image_latest"
...
echo "Updating enrichment app with new image $image"
