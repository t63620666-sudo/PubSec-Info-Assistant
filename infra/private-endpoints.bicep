...
var privateDnsZonesSubscription = !empty(privateDnsZonesSubscriptionId)
...
// DNS Zones
module dnsZones './core/networking/private-dns-zones.bicep' = [
  for privateEndpointConnection in privateEndpointConnections: if (!skipPrivateDnsZones) {
    name: '${privateEndpointConnection.dnsZoneName}-dnszone'
    scope: resourceGroup(privateDnsZonesSubscription, privateDnsZonesResourceGroup)
    params: {
...
var privateEndpointInfo = [
  for (privateEndpointConnection, i) in privateEndpointConnections: map(
    ...
    resourceId => {
      ...
      name: '${last(split(resourceId, '/'))}-${privateEndpointConnection.groupId}'
...
module privateEndpoints './core/networking/private-endpoint.bicep' = [
  for privateEndpointInfo in flatten(privateEndpointInfo): {
    ...
    params: {
      ...
      dnsZoneId: skipPrivateDnsZones ? '' : dnsZones[privateEndpointInfo.dnsZoneIndex].outputs.id
    }
    dependsOn: [dnsZones]
  }
]
...
module monitorDnsZones './core/networking/private-dns-zones.bicep' = [
  for monitorDnsZoneName in monitorDnsZoneNames: if (!skipPrivateDnsZones) {
...
module monitorPrivateEndpoint './core/networking/private-endpoint.bicep' = {
  ...
  params: {
    ...
    privateDnsZoneConfigs: skipPrivateDnsZones
      ? []
      : [
          {
